...
private:
	 ...
	 * @brief Send setpoint to FCU position controller.
	 *
	 * @warning Send only Yaw. ENU frame.
	 */
	void send_yaw_target(const ros::Time &stamp, const Eigen::Affine3d &tr)
	{
		using mavlink::common::MAV_FRAME;
		 ...
		 * If you got similar issue please try update firmware first.
		 */
		const uint16_t ignore_all_except_xyz_y = (1 << 11) | (7 << 6) | (7 << 0);
	...
	void setpoint_cb(const geometry_msgs::PoseStamped::ConstPtr &req)
	{
		...
		if(req->header.frame_id == "YAW"){
			send_yaw_target(req->header.stamp, tr);
		}else{
